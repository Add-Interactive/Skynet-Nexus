// server/scheduler.js
// The cadence engine: three daily editions at 10:00 / 14:00 / 18:00 America/New_York.
...
const DROP_HOURS = [10, 14, 18];        // edition drop times, Eastern
...
// Build a Date for a given Eastern Y/M/D + hour (minute 0).
function etDateToUtc(year, month, day, hour) {
...
function nextDrop(from = new Date()) {
  ...
  for (const h of DROP_HOURS) {
    const cand = etDateToUtc(p.year, p.month, p.day, h);
  ...
  const tp = etParts(t);
  const cand = etDateToUtc(tp.year, tp.month, tp.day, DROP_HOURS[0]);
...
function start() {
  ...
  console.log(`[scheduler] cadence active — drops ${DROP_HOURS.map(h => h + ':00').join(' / ')} ${TZ}; next drop ${info.nextDrop.at} (${info.nextDrop.edition})`);
  releaseDue();
  _timer = setInterval(releaseDue, TICK_MS);
  if (_timer.unref) _timer.unref();
}

module.exports = { start, scheduleInfo, nextDrop, nextEligibleDrop, releaseDue, DROP_HOURS, CUTOFF_MIN, EDITIONS };
