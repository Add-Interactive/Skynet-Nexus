...
async function run() {
  console.log('Starting scheduling trigger for today (July 7th, Evening drop)...');
  const rawDb = new DatabaseSync(DB_PATH);
  
  // 1. Clear ONLY draft/approved stories to preserve scheduled Midday ones!
  const deleteRes = rawDb.prepare("DELETE FROM queued_stories WHERE status IN ('draft', 'approved')").run();
  console.log(`Cleared ${deleteRes.changes} draft/approved items from queue.`);
  
  // 2. Generate fresh drafts
  generateEmergencyDrops();
  console.log('Generated fresh editorial drafts for Evening.');
  
  // 3. Setup dates and times for July 7th Evening drop
  const targetDate = '2026-07-07';
  const edition = 'evening';
  const timeET = `${targetDate}T18:15:00-04:00`; // 6:15 PM ET
  const timeUTC = `${targetDate}T22:15:00.000Z`; // 22:15 UTC
  ...
  for (let i = 0; i < approvedRows.length; i++) {
    ...
    if (customImg) {
      ...
      const imgIndex = (((dayNum * 3) + 2) % 30) + 1; // 2 offset for evening
  ...
  try {
    ...
    execSync('git commit -m "schedule: July 7 Evening drop stories & ComfyUI generated illustrations"', { cwd: ROOT, stdio: 'inherit' });
  ...
  console.log('Today\'s Evening drop setup completed successfully!');
}
